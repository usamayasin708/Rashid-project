...
/*h3 placeholder script starts */
$(document).on('click', 'div.quiz-questionnaire form h3, div.quiz-questionnaire form .quiz-explanation', function(){
	if(!$(this).hasClass("tutor_single_heading_question")){
		if($("div[data-is-lerner=true]").length != 1){
...
/*h3 placeholder script ends */

$(document).on('click', 'div.quiz-questionnaire form input[type=radio]', function(){
		if($("div[data-is-lerner=true]").length > 0){

		}
		else{
			var input_name = $(this).attr('name');
			$(this).closest("form").find("input[name="+input_name+"]").each(function() 
			{ 
...
$(document).on('click', 'div.quiz-questionnaire form input[type=checkbox]', function(){
	...
	$(document).on('click' ,".save-contenting",function(){
			$.ajax({
			type: 'GET',
...
$(document).on('click' ,"#tutor_qoutes_collapse",function(){
	...
	if( collapse ) 
		return;
	$( ".is-design-list" ).prepend('<div class="collapse edit_collection qoutes_collection" id="collapseExamples">'+
...
$(document).on('click' ,"#tutor_image_collapse",function(){
	...
	if( collapse ) 
		return;
	$( ".is-design-list" ).prepend('<div class="collapse edit_collection tutor_main_submenus" id="collapseExample">'+
	'<button type="button" id="close_collaped" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>'+
	'<ul>'+
...
var _snippets_path = _path();
	...
	$(document).on('click' ,"#close_collaped",function(){

	   var items = $('.snippet-item').not(".hide").attr('data-cat');
       if(items)
	   {
		   $(`.is-design-list .snippet-item[data-cat="${items}]`).removeClass('hide');
	   }
	   else
	   {
	    	$(`.is-design-list .snippet-item[data-cat="${items}]`).addClass('hide');
	   }	
	  $('.edit_collection').delay(8000).remove();
	});

	$(document).on('click' ,"#closed_collaped",function(){
		$('.scroll-darker').addClass("acyives");
		$('.acyives').addClass("active");
		$('.edit_collection').delay(8000).remove();
	  });
	 ...
	 /*  $(document).on('click' ,".previewcontent-button",function(e){
		  e.preventDefault();
		  $('.previewcontent').addClass('active');
	  }); */
	
	$(document).on('click' ,"#custom-screen",function(e){
		e.preventDefault();
		var el = $('#main_content').html();
		$("#mobile_content").html(el);
		$('.tutor-save-data').addClass('colhalf');
		$("#myModal").modal('show');	
	});
	$(document).on('click' ,"#close_hide",function(){
		$('.tutor-save-data').removeClass('colhalf');	
		$("#myModal").modal('hide');
	});
 
	$(document).on('click' ,"#medium-screen",function(e){
		e.preventDefault();
		var el = $('#main_content').html();
		$("#tablet_content").html(el);
		$('.column.full,.column.half,.column.third,.column.fourth,.col-md-6').addClass('colhalf');
		$("#myModals").modal('show');
	});
	$(document).on('click' ,"#close_hides",function(){
		$('.column.full,.column.half,.column.third,.column.fourth,.col-md-6').removeClass('colhalf');
		$("#myModals").modal('hide');
	});

   $(document).on('click' ,"#big-screen",function(e){
		e.preventDefault();
		var el = $('#main_content').html();
		$("#desktop_content").html(el);
		$("#myModalies").modal('show');
	});
	$(document).on('click' ,"#close_hidies",function(){
		$("#myModalies").modal('hide');
	}); 
...
$(document).on('click', '.single_placeholder', function() {
	...
	if( $(`.is-design-list .snippet-item[data-cat="${elmId}"]`).length == 0 ){
		...
		$(`.is-design-list .snippet-item[data-cat="${elmId}"]`).removeClass('hide');
	}
   });

   $(document).on('click', '.single_placeholders', function() {
	var elmId = $(this).attr("id");
	$('is-design-list .snippet-item').addClass('hide');
	if( $(`is-design-list .snippet-item[data-cat="${elmId}"]`).length == 0 ){
		var items = data_basic.snippets.filter( x => x.category == elmId  );
        for(var i=0; i < items.length; i++){
			let html = `<div class="snippet-item" data-id="${i+1}" data-cat="${elmId}"><img alt="Snippet" src="../assets/minimalist-blocks/${items[i].thumbnail}"><span class="is-overlay"></span></div>`;
		var preval = $(`.is-design-list`).append(html);
	}
	}else{
		$(`is-design-list .snippet-item[data-cat="${elmId}"]`).removeClass('hide');
...
// 	var type_question_1 = document.getElementById("type_question_1").value;
// 	let ans1  = document.forms[0];
// 	let txt = "";
// 	let i;
...
// 		if(ans1[i].checked)
// 		{
// 			txt = txt + ans1[i].value + "";
// 		}
// 	}
// 	document.getElementById("result").value = "Result is:" + type_question_1;
// 	localStorage.setItem('Result is: ', type_question_1);
// 	localStorage.setItem('Result is: ', txt);
...
// function extra_customer(){
// 	$('.customer_records').clone().appendTo('.customer_records_dynamic');
// 	$('.customer_records_dynamic .customer_records').addClass('single remove');
// 	$('.single .extra-fields-customer').remove();
// 	$('.single').append('<a href="#" class="btn btn-success remove-field btn-remove-customer"><i class="fa fa-minus" aria-hidden="true"></i></a>');
// 	$('.single').append('<a href="#" onclick="extra_customer()" class="btn btn-success btn-extra extra_customer"><i class="fa fa-plus" aria-hidden="true"></i></a>');
...
// 	$('.customer_records_dynamic label').each(function() {
// 		var count = 0;
// 		var id = $(this).attr("class");
// 		$(this).attr('class', id + count );
// 		$(this).attr('for', id + count );
...
$(document).on('click', '.btn-remove-option-multi', function(){

	var data_id = $(this).attr('data-id');
	$(this).closest('form').find('span[data-id='+data_id+']').remove();
	//data-id="9560"
	$(this).closest('.multi-blank-div').remove();
	
	//
 });

$(document).on('click', '#learner_close_advertis', function(){
   $('.learner-advertise').remove();
});

$(document).on('click', '#check_show_item', function() {
	$("#check_response_answer").css("display","block");
});
...
$(document).on('click','.option_boolean_ones .radio_mul_checked',function(each){
	
		var id =	$(this).attr('id');	
		$('.sgnl_blank_ip1').val(id);
		if($('.sgnl_blank_ip2').val() == ""){
			$('.sgnl_blank_ip2').val(id);
		}
 });
 


$(document).on('click','.radio-checked',function(each){
	var id =	$(this).attr('id');	
	$('.its-answer').val(id);
	$('.its-boolean-answer').val(id);
 });
 
 $(document).on('click','.option_boolean_ones .radio_mul_checked',function(each){	
		/*option_boolean_ones questionnaire-multi-choice*/
		var id =	$(this).attr('id');			
		if($(this).closest("div[data-is-lerner=true]").length > 0){
			$('.multiple_bbolean_one').val(id);
		}
		else{
			console.log("na");
			$('.multiple_bbolean').val(id);
		}
 ...
 $(document).on('click','.cbox_option',function(each){
	var id = [];
	var is_exists = $(this).closest("div[data-is-lerner=true]").length;
	$(this).closest('form').find('input').each(function(){
	  if($(this).prop("checked") == true){
	   	id.push($(this).attr('id'));
		if(is_exists > 0){
			$('.multis_boxes_answers').val(id);
		}
		else{
			$('.multiple_boxes_answers').val(id);
...
$(document).on('click', '.insertsymbol-button', function() {
	$(".logo_tutor").css("z-index","999");  
});
$(document).on('click', '.is-side-close', function() {
	$(".logo_tutor").css("z-index","999999999");  
});
$(document).on('click', '.btn-closeing', function() {
...
$(document).on('click', '.correct-answer-btn', function() {
	...
	if($(this).closest('form').find('.multi-blank-div').length > 0 ){
		...
		var main_btn = $(this);
		$(this).closest('form').find('.multi-blank-div').each(function(){
			var answers = $(this).closest('.multi-blank-div').find('input').attr('data-multiblank-answers');
			var adm_ans = $("input[data-multiblank-answers="+answers+"]").val();

			var answer = $(this).closest('.multi-blank-div').find('input').attr('data-multiblank-answers');
			var lerner_ans = $("input[data-multiblank-answer="+answer+"]").val();
			if(adm_ans != lerner_ans){

				$(this).closest('.multi-blank-div').find('div.mcq-checked').each(function(){
						
						var radio_val = $(this).closest('div.mcq-checked').find('input').attr('id');

						if(radio_val == adm_ans){
							$(this).closest('div.mcq-checked').find('label').addClass('correctAnser');
						}

						if(radio_val != adm_ans){
							// $(this).closest('div.mcq-checked').find('label').addClass('checking');
							// labels =   $('.multiple_bboolean').parents('.option_boolean_ones').find('.form-group .inputGrouping label');
							// labels.addClass('checking');
							$(this).closest('div.mcq-checked').find('label').addClass('checking');
						//	$(this).closest('form').find('.'+slectedOpts).removeClass('checking');
							
						}
				});
				// mcq-checked

				$(".multiple_response_answer").css("display","block");
				// $(this).closest('.multi-blank-div').find('.'+adm_ans).addClass('checking');
				// $(this).closest('.multi-blank-div').find('.'+adm_ans).addClass('correctAnser');
	...
	//correct textarea
	else if($(this).closest('form').find('textarea').length > 0 ){
		var textArea = $(this).closest('form').find('textarea').val();
		$(this).closest('form').find(".boolean_question_response").css("display","block");
	}
	//Correct
	else if($(this).closest('form').find('.multiple_bbolean_one').length > 0){
		var slectedOpionts = $(this).closest('form').find('.multiple_bbolean_one').val();
		var slectedOpts = $(this).closest('form').find('.multiple_bboolean').val();
		if(slectedOpts === slectedOpionts)
		{
			$(this).closest('form').find('.'+slectedOpts).addClass('correctAnser');
		}
		else
		{

			$(this).closest('form').find('div.choiceGrouping').each(function(){
						
				var radio_val = $(this).closest('div.choiceGrouping').find('input').attr('id');
				console.log("aaaaa", radio_val);
				if(radio_val == slectedOpionts){
					$(this).closest('div.choiceGrouping').find('label').addClass('correctAnser');
				}
				...
				if(radio_val != slectedOpionts){
					...
					// labels =   $('.multiple_bboolean').parents('.option_boolean_ones').find('.form-group .inputGrouping label');
					// labels.addClass('checking');
					$(this).closest('div.choiceGrouping').find('label').addClass('checking');
				//	$(this).closest('form').find('.'+slectedOpts).removeClass('checking');
		...
		// 	$(this).closest('div.mcq-checked').find('label').addClass('checking');
		// 	//
		//   getNames  	=    $('#'+slectedOpts).attr('name');  
		//   labels  		=   $('.multiple_bboolean').parents('.option_boolean_ones').find('.form-group .inputGrouping label');
		//   labels.addClass('checking');
		//   $(this).closest('form').find('.'+slectedOpts).removeClass('checking');
		//   $(this).closest('form').find('.'+slectedOpts).addClass('correctAns');
		  $(this).closest('form').find(".check_question_response").css("display","block");
		}
	}
	//new
	else if($(this).closest('form').find('.sgnl_blank_ip2').length > 0){
		var slectedOpionts = $(this).closest('form').find('.sgnl_blank_ip2').val();
		var slectedOpts = $(this).closest('form').find('.sgnl_blank_ip1').val();
		console.log("me", slectedOpts , slectedOpionts);
		if(slectedOpts === slectedOpionts)
		{
			$(this).closest('form').find('.'+slectedOpts).addClass('correctAnser');
		}
		else
		{
			...
			$(this).closest('form').find('div.fillGrouping').each(function(){
						
				var radio_val = $(this).closest('div.fillGrouping').find('input').attr('id');
				console.log("aaaaa", radio_val);
				if(radio_val == slectedOpionts){
					$(this).closest('div.fillGrouping').find('label').addClass('correctAnser');
				}
				...
				if(radio_val != slectedOpionts){
					...
					// labels =   $('.multiple_bboolean').parents('.option_boolean_ones').find('.form-group .inputGrouping label');
					// labels.addClass('checking');
					$(this).closest('div.fillGrouping').find('label').addClass('checking');
				//	$(this).closest('form').find('.'+slectedOpts).removeClass('checking');
		...
		//   getNames  =    $('#'+slectedOpts).attr('name');  
		//   labels    =   $(this).closest('form').find('.form-group .inputGrouping label');
		//   labels.addClass('checking');
		//   $(this).closest('form').find('.'+slectedOpts).removeClass('checking');
		//   $(this).closest('form').find('.'+slectedOpts).addClass('correctAns');
		  $(this).closest('form').find(".check_question_response").css("display","block");
	...
	//Corrected
	else if($(this).closest('form').find('.its-boolean-answer').length > 0)
	{
		var slectedOpiont = $(this).closest('form').find('.its-boolean-answer').val();
		var slectedOpt = $(this).closest('form').find('.its-org').val();
		
		if(slectedOpt === slectedOpiont)
		{
			$(this).closest('form').find('.'+slectedOpt).addClass('correctAnser');
		}
		else
		{
			getName  =    $(this).closest('form').find('#'+slectedOpt).attr('name'); 
			labels  =     $(this).closest('form').find('.'+getName).parents('.options_booleans').find('.inputGrouping label');
			labels.addClass('checking');
			$(this).closest('form').find('.'+slectedOpt).removeClass('checking');
			$(this).closest('form').find('.'+slectedOpt).addClass('correctAns');
			$(this).closest('form').find(".boolean_question_response").css("display","block");
		}
	}
		//check_multiple_response_answer
    	else if( $(this).closest('form').find('.multiple_boxes_answers').length > 0 )
		{
		var slectedOpiontings = $(this).closest('form').find('.multiple_boxes_answers').val();
		var slectedOptings = $(this).closest('form').find('.multis_boxes_answers').val();
		if(slectedOptings === slectedOpiontings)
		{
			var  arr =   slectedOptings.split(',');
			arr.forEach(element => {
			 $(this).closest('form').find('.'+element).removeClass('checking');
			 $(this).closest('form').find('.'+element).addClass('correctAns');
			});
		}
		else
		{
		  getNameis = $(this).closest('form').find('#'+slectedOptings).attr('name');  
		  labels = $(this).closest('form').find('.multis_boxes_answers').parents('.multiple_checked_boxing').find('.form-group .inputGrouping label');
		  labels.addClass('checking');
		   var  arr =   slectedOptings.split(',');
		   arr.forEach(element => {
			$(this).closest('form').find('.'+element).removeClass('checking');
	...
	else if($(this).closest('form').find('.mcqing-answers').length > 0){
		var selectedValue =  $(this).closest('form').find('.mcqing-answers');
		var slectedOptinges =  $(this).closest('form').find('.multi_mcqting_answer');
			selectedValue.each(function( index ) {
			var asnweer  = $(this).closest('.single_closest').find('.multi_mcqting_answer').val();
			var asnweers  = $(this).closest('.single_closest').find('.mcqing-answers').val();
			if(asnweer === asnweers)
			{
			$('.'+asnweer).addClass('correctAnser');
			}

				else
				{
					$(this).closest('.single_closest').find('label').addClass('checking');
					ansLabel   =	$(this).val();
					$(this).closest('form').find('.'+asnweer).removeClass('checking');
					$(this).closest('form').find('.'+asnweer).addClass('correctAns');
					$(this).closest('form').find(".question.response").css("display","block");
				}
			});
		}
...
//Humayun
$(document).on('click', '.mcq_checked', function(){

	if($("div[data-is-lerner=true]").length > 0){
		if($(this).prop("checked") == true){
			var id =	$(this).attr('id');	
			var radio_btn_name = $(this).attr('name');
			//var val1 = $(this).closest('div.multi-blank-div').find("input[data-multiblank-answers="+radio_btn_name+"]").val(id);
			var val2 = $(this).closest('div.multi-blank-div').find("input[data-multiblank-answer="+radio_btn_name+"]").val(id);
		}
	}
	else{
...
function searchQuestion(nameKey, myArray){
 ...
 $(document).on('click', '.quiz-submit-answer', function(event) {
	event.preventDefault();

	var is_submit = true;
	if($(this).closest('form').find('textarea').length > 0){
		$(this).closest('form').find('textarea').each(function(){
			if($(this).val() == ""){
				var div_id = $(this).closest('form').find('div.error').attr("id");
				printError(div_id, "This field is required.");
				$(this).closest('form').find('div.error').css('color','red');
				is_submit = false;			
				return '';
			}
		 });
	}
	else if($(this).closest('form').find('input').length > 0){
		var iz_checked = false;
		$(this).closest('form').find('input').each(function(){
		   if($(this).is(':checked')){ iz_checked = true; }
		});
		if ( iz_checked == false ){
			var div_id = $(this).closest('form').find('div.error').attr("id");
			printError(div_id, "Please select your required field");
			$(this).closest('form').find('div.error').css('color','red');
			is_submit = false;
			return '';
		}
	...
	var questions 	= localStorage.getItem('questionsarr');

	//let ref_id		= localStorage.getItem('question_ref_id');
	var ref_id 	= $(this).closest('form').find('input[name=question_ref_id]').val();
	var label_values = [];
	var is_input_check_radio = false;
	$(this).closest('form').find('input').each(function() {
		is_input_check_radio = true;
		if($(this).is(":checked")){
			var values = $(this).closest('div.inputGrouping').text();
	...
	for( var i = 0; i < questions.length; i++ ) {
		if(questions[i].key.ref_id == ref_id ){

			qId = questions[i].key._id;
			if(questions[i].key.options.length > 0){
				for( var j = 0; j < questions[i].key.options.length; j++ ) {
					var option_name = questions[i].key.options[j].option;
					if(questions[i].key.options[j].option.length >0 && jQuery.inArray( option_name, label_values )){
						var opt_id = questions[i].key.options[j]._id;
						 var optionsAnswers = {"option_id": opt_id, 'is_correct': 1};
						 options_answers.push(optionsAnswers);
					}
				}
			}
			if(options_answers.length > 0){
				var answerQuestions = {"question_id": qId, 'answer': options_answers}
			}
			else{
				var textarea_value = $(this).closest('form').find('textarea').val();
				var answerQuestions = {"question_id": qId, 'answer': [{"text":textarea_value}]}
			}
			answer_questions.push(answerQuestions);
		}
	}

var formData = {
	"content_id" : parseInt(contentId),
	"answers"    : JSON.stringify(answer_questions),
...
$.ajax({
	type: "POST",
	url: "https://api.thetutor.me/api/learning-material-quiz/filled-quiz",
	data: formData,
	dataType: "json",
	headers:{
		"X-Auth-Token": '',
  ...
  event.preventDefault();
		...
		// Toast Type
		$(this).parent().parent().parent().find("button:eq(1)").addClass('bolean_view_correct_option');
		$(this).parent().parent().parent().find("button:eq(1)").prop('disabled',false);
		$(this).parent().parent().parent().find("button:eq(1)").css('background','#198754');
...
$(document).on('click', '#add_current', function() {
		$('#tutor_add_page_modal').modal('show');
		$(".is-ui").css("z-index","999");
		$(".pagination").css("z-index","999");
		$("#divSnippetList").css("z-index","999");
		$(".logo_tutor").css("z-index","999");
		$('.tutor-menu-content').css("z-index","999");
});

$(document).on('click', '#edit_current', function() {
	$('#tutor_edit_page_modal').modal('show');
	$(".is-ui").css("z-index","999");
	$(".pagination").css("z-index","999");
	$("#divSnippetList").css("z-index","999");
	$(".logo_tutor").css("z-index","999");
...
$(document).on('click', '#tutor_close_page', function() {
	$('#tutor_add_page_modal').modal('hide');
	$(".is-ui").css("z-index","999999999");
	$(".pagination").css("z-index","99999999");
	$("#divSnippetList").css("z-index","10003");
	$('.tutor-menu-content').css("z-index","999999999");
	$(".logo_tutor").css("z-index","999999999");
});

$(document).on('click', '#tutor_closing_page', function() {
	$('#tutor_edit_page_modal').modal('hide');
	$(".is-ui").css("z-index","999999999");
	$(".pagination").css("z-index","99999999");
	$("#divSnippetList").css("z-index","10003");
	$('.tutor-menu-content').css("z-index","999999999");
...
$(document).on('click', '.btn-add-option-multi', function(e) {

	// var input_count = $(this).closest('form').find('div.mobile_screen_size').length;
	// var new_input = input_count+1+Math.floor(Math.random() * 10100);
	// var new_input_text = input_count+1;	

	var input_name = $(this).closest('.multi-blank-div').find('input[type=radio]').attr('name');

	var last_row = $(this).closest('.multi-blank-div').find('div.add-more-row:last').find('input.data-row-number').val();
	var new_input_text = Number(last_row) + 1;
	var new_input = new_input_text+Math.floor(Math.random() * 10100);
	


	var html = '<div class="row add-more-row">'+
	'<input type="hidden" name="data_row_number" class="data-row-number" value="'+new_input_text+'" />'+
	'<div class="col-md-10 offset-md-1 mobile_screen_size tutor_blank_option tutor_multiple_heading">' +
	'<div class="form-group customer_records">'+
	'<div class="form-group inputGrouping mcq-checked" data-editable="false" contenteditable="false">'+
	'<input id="'+new_input+'" name="'+input_name+'" type="radio" data-id ="2" class="mcq_checked"/>'+
	'<label class="'+new_input+'" for="'+new_input+'" data-editable="true" contenteditable="true">Option '+new_input_text+'</label>'+
	'</div>'+	
	'</div>'+
	'</div>'+
	
	'<div class="col-md-2">' +
	'<div class="extra_customer_one">'+
	'<a href="#" class="btn btn-add-option btn-add-option-multi"><i class="fa fa-plus" aria-hidden="true"></i></a>'+
	'<a href="#" class="btn btn-danger remove-field btn-remove-customer btn_records_one" contenteditable="false" data-editable="false"><i class="fa fa-minus" aria-hidden="true"></i></a>'+
	'</div>'+
	'<input type="hidden" value = 0  class="single-check-box">'+
	'<div class="customer_records_dynamic_one"></div>'+
	'</div>'+
	'</div>'+
	'</div>';

	$(this).closest('div.row').after(html);
				
	$(this).closest('.multi-blank-div').find('.btn-add-option-multi').each(function() { $(this).hide(); });
	$(this).closest('.multi-blank-div').find('.btn-add-option-multi:last').show();
	$(this).hide();
...
$(document).on('click', '.btn-remove-customer', function(e) {

	e.preventDefault();
	var ff = $(this).closest('form');
	var gg = $(this).closest('.multi-blank-div');

	console.log("gg", gg);
	console.log("ff", ff);
	$(this).closest(".row").remove();
	
	gg.find('.btn-add-option-multi').hide();
	gg.find('.btn-add-option-multi:last').show();	

	//ff.closest('form').find('.btn-add-option-multi').hide();
	//ff.closest('form').find('.btn-add-option-multi:last').show();	

});

   
   $(document).on('click', '.add_new_blank', function(e) {


	var input_count = $(this).closest('form').find('div.multi-blank-div').length;
	var new_input = input_count+1+Math.floor(Math.random() * 10100);
	var new_input_text = input_count+1;	

	var blank_text = input_count+1;	
	var blank_name = 'name_'+blank_text;
	var new_input1 = new_input+'1';
	var new_input2 = new_input+'2';

	var prevval 		= $(this).closest( "form" ).find('.blank-heading-attach').html();

	$(this).closest( "form" ).find('.blank-heading-attach').html(prevval + '<span contentEditable="false" contenteditable="false" data-id= ' + new_input + ' class="line_remove_blan"><span class="iconify tutor_iconify" contentEditable="false" contenteditable="false"  data-icon="ant-design:line-outlined"></span></span> ');   

		var html = '<div class="row multi-blank-div">'+
		'<input type="hidden" value="" data-multiblank-answers="'+blank_name+'" name="multiblank-answers['+blank_name+']" />'+
		'<input type="hidden" value="" data-multiblank-answer="'+blank_name+'" name="multiblank-answer['+blank_name+']" />'+
		'<div class="col-md-9 offset-md-1 mobile_screen_size tutor_blank_option single_closest tutor_multiple_heading">' +
		'<div data-editable="false" contenteditable="false">'+
		'<h5 class="crt_option numbering_sort" contenteditable="true" data-editable="true">Blank '+new_input_text+'</h5>'+
		
		'</div>'+	
		'</div>'+
		'<div class="col-md-2">'+
		'<a href="#" class="btn btn-danger remove-field btn-remove-customer-two btn-remove-option-multi"  data-id='+new_input +' contenteditable="false" data-editable="false"><i class="fa fa-minus" aria-hidden="true"></i></a>'+
		
		'</div>'+	

		'<div class="row add-more-row">'+
		'<input type="hidden" name="data_row_number" class="data-row-number" value="1" />'+
		'<div class="col-md-10 offset-md-1 mobile_screen_size tutor_blank_option tutor_multiple_heading">' +
		'<div class="form-group">'+					
		'<div class="form-group inputGrouping mcq-checked"  data-editable="false" contenteditable="false">'+
		'<input id="'+new_input1+'" name="'+blank_name+'" type="radio" data-id ="'+new_input1+'" class="mcq_checked"/>'+
		'<label class="'+new_input1+'" for="'+new_input1+'" data-editable="true" contenteditable="true">Option 1</label>'+
		'</div>'+	
		'</div>'+
		'</div>'+
		'</div>'+
		'<div class="row add-more-row">'+
		'<input type="hidden" name="data_row_number" class="data-row-number" value="2" />'+
		'<div class="col-md-10 offset-md-1 mobile_screen_size tutor_blank_option tutor_multiple_heading">' +
		'<div class="form-group customer_records">'+
		'<div class="form-group inputGrouping mcq-checked" data-editable="false" contenteditable="false">'+
		'<input id="'+new_input2+'" name="'+blank_name+'" type="radio" data-id ="2" class="mcq_checked"/>'+
		'<label class="'+new_input2+'" for="'+new_input2+'" data-editable="true" contenteditable="true">Option 2</label>'+
		'</div>'+	
		'</div>'+
		'</div>'+
		'<div class="col-md-2">' +
		'<div class="extra_customer_one"><a href="#" class="btn btn-add-option btn-add-option-multi"><i class="fa fa-plus" aria-hidden="true"></i></a></div>'+
		'<input type="hidden" value = 0  class="single-check-box">'+
		'<div class="customer_records_dynamic_one"></div>'+
		...
		$(this).closest('form').find('div.multi-blank-div:last').after(html);
			
		//$(this).closest('form').find('.add-checkbox-row').each(function() { $(this).hide(); });
		//$(this).closest('form').find('.add-checkbox-row:last').show();

   });

   $(document).on('click', '.btn-tutor-blank', function(e) {
	...
	$(this)
		...
		.change(function() {
			...
			reader.onload = function(e) {
				...
				$(previewImg).attr("class", 'page-thumb');
...
$(document).on('click', '.single_blank_add', function(e) {
  
		
		var last_row = $(this).closest('form').find('div.add-more-row:last').find('input.data-row-number').val();
		var new_input_text = Number(last_row) + 1;
		var new_input = new_input_text+Math.floor(Math.random() * 10100);
		...
		var html = '<div class="row add-more-row">' +
		'<input type="hidden" name="data_row_number" class="data-row-number" value="'+new_input_text+'" />'+
		'<div class="col-md-9 offset-md-1" id="tio_clone">' +	
		'<div class="form-group">'+
		'<div class="inputGrouping fillGrouping" data-editable="false" contenteditable="false">'+
		'<input id="radio2-blank-'+new_input+'" name="radio" type="radio" data-id="'+new_input_text+'" class="radio_mul_checked"/>'+
		'<label class="radio2-blank" for="radio2-blank-'+new_input+'" data-editable="true" contenteditable="true">Option '+new_input_text+'</label>'+
		'</div>'+
		'</div>'+
		'</div>'+
		'<div class="col-md-2">' +	
		'<div class="tutor_single_button">'+
		'<a class="btn btn-add-option single_blank_add" type="button"><i class="fa fa-plus" aria-hidden="true"></i></a>'+
		'<a class="btn btn-danger delete-input-row"  contenteditable="false" data-editable="false"><i class="fa fa-minus" aria-hidden="true"></i></a>'+
		'</div>'+
		'</div>'+
		'<div class="error col-md-9 mobile_screen_size offset-md-1" id='+Math.floor(Math.random() * 1000)+'></div>'+
		'</div>'+
		'</div>';
		...
		$(this).closest('form').find('.single_blank_add').each(function() { $(this).hide(); });
		$(this).closest('form').find('.single_blank_add:last').show();

		return;
	});
	
	$(document).on('click', '.delete-input-row', function(e) {
			//$(this).closest(".inputGrouping").remove();
			//

			e.preventDefault();
			var ff = $(this).closest('form');
			$(this).closest(".row").remove();
			ff.closest('form').find('.single_blank_add').hide();
			ff.closest('form').find('.single_blank_add:last').show();	

		});
 
		
		$(document).on('click', '.add-checkbox-row', function(e) {
			...
			var html =	'<div class="row">' +
				'<div class="col-md-9 offset-md-1 mobile_screen_size add-more-row" id="tutor_checked_appen">' +	
				'<input type="hidden" name="data_row_number" class="data-row-number" value="'+new_input_text+'" />'+
				'<div class="form-group">'+
				'<div class="inputGrouping checkGrouping"  data-editable="false" contenteditable="false">'+
				'<input id="option'+new_input+'" name="option" type="checkbox" value="Option '+new_input+'" class="cbox_option" />'+
				'<label class="option2" for="option'+new_input+'"  data-editable="true" contenteditable="true">Option '+new_input_text+'</label>'+
				'</div>'+
				'</div>'+
				'</div>'+
				...
				'<div class="col-md-2">' +	
				'<div class="tutor_single_checked_button" contenteditable="false" data-editable="false">'+
				'<a class="btn btn-add-option add-checkbox-row"><i class="fa fa-plus" aria-hidden="true"></i></a>'+
				'<a class="btn btn-danger delete-check-row" contenteditable="false" data-editable="false"><i class="fa fa-minus" aria-hidden="true"></i></a>'+
				'</div>';
			...
			$(document).on("drop", '.snippet-item' , function(){
				if($(this).data('cat') == '120'){

					$(".is-modal.iframelink").addClass('active');
					//$('.is-iframe-tool button').trigger('click');
				
					// var button = $('button').data('title');
					// $('.iframelink').addClass('active');
					// $('.input-src').css('display','block');
					// $('.is-modal-content').addClass('minimize');								
				}
			});

			$(document).on('click', '.input-ok.classic-primary', function(e) {
				$('.iframelink').removeClass('active');
			});


	$(document).on('click', '.delete-check-row', function(e) {
			e.preventDefault();
			var ff = $(this).closest('form');
			$(this).closest(".row").remove();
			ff.closest('form').find('.add-checkbox-row').hide();
			ff.closest('form').find('.add-checkbox-row:last').show();	
		});
		...
		$(document).on('click', '.add-choice-row', function(e) {
			...
			var new_input = new_input_text+Math.floor(Math.random() * 10100);
			var html = '<div class="row">'+ 
			'<div class="col-md-9 mobile_screen_size add-more-row offset-md-1">' +
			'<input type="hidden" name="data_row_number" class="data-row-number" value="'+new_input_text+'" />'+
			'<div class="form-group">'+ 
			'<div class="inputGrouping choiceGrouping" contenteditable="false" data-editable="false">'+
			'<input id="radio'+new_input+'" name="radio" type="radio" data-id ="'+new_input+'" class="radio_mul_checked"/>'+
			'<label class="radio2" for="radio'+new_input+'" contenteditable="true" data-editable="true">Option '+new_input_text+'</label>'+
			'</div>'+
			'</div>'+
			'</div>'+
			'<div class="col-md-2">' +	
			'<div class="tutor_single_choice_button">'+
			'<a class="btn btn-add-option add-choice-row add-choice-row-js" type="button"><i class="fa fa-plus" aria-hidden="true"></i></a>'+
			'<a class="btn btn-danger delete-choice-row" type="button" contenteditable="false" data-editable="false"><i class="fa fa-minus" aria-hidden="true"></i></a>'+
			'</div>'+
			'</div>'+
			'</div>';
			...
			$(this).closest('form').find('.add-choice-row').each(function() { $(this).hide(); });
			$(this).closest('form').find('.add-choice-row:last').show();
			return;
			...
			$(document).on('click', '.delete-choice-row', function(e) {
				...
				var ff = $(this).closest('form');
				$(this).closest(".row").remove();
				ff.closest('form').find('.add-choice-row').hide();
				ff.closest('form').find('.add-choice-row:last').show();
			});
...
{
	var formData = {
		question_num : document.getElementById("question_num").innerText,
		type_question: $("#type_question").val(),
	  };
	  $.ajax({
		type: "POST",
		url: "http://localhost:5000/api/posts/tutorsave",
		data: formData,
		dataType: "json",
		encode: true,
	  }).done(function (data) {
...
function editFunction(id)
{
	...
	$.ajax({
		type: 'get',
		url: 'http://localhost:5000/api/posts/tutor/'+id_val,
		dataType:"json",
		encode: true,
		success: function (data) {
...
var data_basic = {
	...
	{
		'thumbnail': 'preview/Fill-in-the-Blanks-Tests.png',
		'category': '126',
		'html':
			'<div class="row clearfix" data-editable="false" contenteditable="false">' +		
			  '<div class="col-md-12">' +
			   '<div class="acie_ment subjective_question quiz-questionnaire" id='+Math.floor(Math.random() * 10100)+'>'+
			   '<form name="attempt_blank_answer" id="add_new_blank_answer" class="questionnaire-question-text">'+
			   '<input type="hidden" value="0" class="tutor_hide_question_option" />'+
			   '<input type="hidden" value="" name="question_ref_id"  />'+
			   '<div class="form-group">'+
			   '<div class="hidden-group">'+
			   '<h1 id="question_num" contenteditable="false" data-editable="false">Short/Long Text Answer Question Template</h1>'+
			   '<input type="hidden" name="question_num"/>'+
			   '</div>'+
			   '</div>'+
			   '<div class="form-group" data-editable="false" contenteditable="false">'+
			   '<h3 rows="1" id="question" class="blank-heading-attach tutor_question_answer question" contenteditable="true" data-editable="true" contentH3editable="true" data-text="Enter Question text Here ?">Enter Question text Here ?</h3>'+
			   '</div>'+
			   '<div class="form-group">'+
			   '<textarea type="text" name="attemp_answer" id="attemp_answer" class="blank-heading-attach attemp_answer" placeholder="Write A Answer" required></textarea>'+
			   '</div>'+
			   '<div class="error" id='+Math.floor(Math.random() * 1000)+'></div>'+
/* 			   '<div class="form-group">'+
			   '<h3 name="correct_answer" id="correct_answer" class="blank-heading-attach correct_answer">Enter Answer Text Here (Will be made visible upon Learners answer submission)</h3>'+
			   '</div>'+ */
			   '<div class="form-group" data-editable="false" contenteditable="false">'+
			   '<div id="boolean_response_answer" class="boolean_question_response quiz-explanation" contenteditable="true" data-editable="true" contentDiveditable="true" data-text="Explanation: Enter the explanation for the correct answer (Will be made visible upon learner answer submission)">Explanation: Enter the explanation for the correct answer (Will be made visible upon learner answer submission)'+
			   '</div>'+
			   '</div>'+
			
			   '<div class="form-group" data-editable="false" contenteditable="false">'+
			   '<ul class="sub_view_answer">'+
			   '<li>'+
			   '<div id="toasts"></div>'+
			   '<button id="cor_success" type="button" class="quiz-submit-answer">Submit Answer</button>'+
			   '</li>'+
			   '<li>'+
			   '<button id="show_item" class="corting_answer disabled correct-answer-btn" type="button" disabled>View Correct Answer</button>'+				   
			   '</li>'+
			   '</ul>'+
			   '</div>'+
			   '</form>'+
	...
	{
		'thumbnail': 'preview/images.png',
		'category': '126',
		'html':
	       '<div class="row clearfix" data-editable="false" contenteditable="false">' +		
		   '<div class="col-md-12">' +
			'<div class="quiz-questionnaire" id='+Math.floor(Math.random() * 100000)+' data-editable="false" contenteditable="false">'+
			'<form name="contactForm" class="select_option_mc options_booleans questionnaire-option">'+
			'<input type="hidden" value=0 class="tutor_hide_bolean_option" />'+
			'<input type="hidden" value="" name="question_ref_id"  />'+
			'<div class="row clearfix">' +
			'<div class="col-md-12">' +
				'<div class="acie_ment option_boolean">' + 
			   '<h1 id="question-type-options" contenteditable="false" data-editable="false">Single Choice (Yes / No or True / False) Question Templates</h1>' +
			   '<div class="form-group" data-editable="false" contenteditable="false">'+
			   '<h3 rows="1" id="question-texting" class="quest-heading-attach" contenteditable="true" data-editable="true" contentH3editable="true" data-text="Enter Question text Here ?">Enter Question text Here ?</h3>'+
			   '</div>'+
				'</div>' +
				'</div>' +
				'</div>' +
				'<div class="row">' +
				'<div class="col-md-10 offset-md-1 mobile_screen_size tutor_blank_option">' +	
				'<div class="form-group">'+
				'<div class="inputGrouping" data-editable="false" contenteditable="false">'+
				'<input id="radOne" value="yes" name="radOne" type="radio" class="radio-checked"/>'+
				'<label class="radOne" for="radOne" data-editable="true" contenteditable="true">Yes</label>'+
				'</div>'+	
				
				'<div class="inputGrouping" data-editable="false" contenteditable="false">'+
				'<input id="radTwo" value="no" name="radOne" type="radio" class="radio-checked"/>'+
				'<label class="radTwo" for="radTwo"  data-editable="true" contenteditable="true">No</label>'+
				'</div>'+	

				'<div class="error" id='+Math.floor(Math.random() * 1000)+'></div>'+

				 '<input class = "its-answer" type="hidden">'+
				 '<input class = "its-boolean-answer" type="hidden">'+

				'<div class="form-group"  data-editable="false" contenteditable="false">'+
				'<div  data-editable="true" contenteditable="true" id="boolean_response_answer" class="boolean_question_response quiz-explanation" contentDiveditable="true" data-text="Explanation: Enter the explanation for the correct answer (Will be made visible upon learner answer submission)">Explanation: Enter the explanation for the correct answer (Will be made visible upon learner answer submission)'+
				'</div>'+
				'</div>'+
				'</div>'+
				'</div>'+
				'<div class="form-group bolean_save_answer"  data-editable="false" contenteditable="false">'+
				'<ul class="boolean_view_answer">'+
				'<li>'+
				'<div id="toasts"></div>'+
				'<button id="radios_success" class="submit_mcq_answer bol_transp quiz-submit-answer" type="submit"><i class="fa fa-save"></i> Submit Answer</button>'+
				'</li>'+
				'<li>'+
				'<button id="boolean_show_item" class="bol_transp bollean-transpot disabled correct-answer-btn" type="button" disabled><i class="fa fa-eye"></i> View Correct Answer</button>'+				   
				'</li>'+
				'</ul>'+
				'</div>'+
				'</form>'+
	...
	{
		'thumbnail': 'preview/Matching-Tests.png',
		'category': '126',
		'html':
			'<div class="row clearfix" contenteditable="false" data-editable="false">' +		
			  '<div class="col-md-12">' +
			   '<div class="acie_ment option_boolean_one quiz-questionnaire"  id='+Math.floor(Math.random() * 1100)+'>' +
			 
			   '<form class="option_boolean_ones questionnaire-multi-choice">'+
			   '<input type="hidden" value=0 class="tutor_hide_fill_option" />'+
			   '<input type="hidden" value="" name="question_ref_id"  />'+
			   '<div class="form-group">'+
			   '<div class="hidden-group">'+
			   '<h1 id="question_num" contenteditable="false" data-editable="false">Multiple Choice Question Template</h1>'+
			   '<input type="hidden" name="question_num"/>'+
			   '</div>'+
			   '</div>'+
			 
			   '<div class="form-group" contenteditable="false" data-editable="false">'+
			   '<h3 class="blank-heading-attach" rows="1" contenteditable="true" data-editable="true" contentH3editable="true" data-text="Enter Question text Here ?">Enter Question text Here ?</h3>'+
			   '</div>'+

			   '<div class="tutor_multiple_choice_option">'+
			   '<div class="row">' +
			   '<div class="col-md-9 mobile_screen_size add-more-row offset-md-1">' +	
			   '<input type="hidden" name="data_row_number" class="data-row-number" value="1" />'+
			   '<div class="form-group">'+
				'<div class="inputGrouping choiceGrouping" contenteditable="false" data-editable="false">'+
				'<input id="radio1" name="radio" type="radio" data-id ="1" class="radio_mul_checked" />'+
				'<label class="radio1" for="radio1" contenteditable="true" data-editable="true">Option 1</label>'+
				'</div>'+	
				'</div>'+
				'</div>'+
				'</div>'+
				...
				'<div class="row">' +
				'<div class="col-md-9 mobile_screen_size add-more-row offset-md-1" id="tutor_choice_clone">' +	
				'<input type="hidden" name="data_row_number" class="data-row-number" value="2" />'+
				'<div class="form-group">'+ 
				'<div class="inputGrouping choiceGrouping" contenteditable="false" data-editable="false">'+
				'<input id="radio2" name="radio" type="radio" data-id ="2" class="radio_mul_checked"/>'+
				'<label class="radio2" for="radio2" contenteditable="true" data-editable="true">Option 2</label>'+
				'</div>'+
				'</div>'+
				'</div>'+
				...
				'<div class="col-md-2">' +	
				'<div class="tutor_single_choice_button">'+
				'<a class="btn btn-add-option add-choice-row"><i class="fa fa-plus" aria-hidden="true"></i></a>'+
				'</div>'+
				'</div>'+
				...
				'<input class="multiple_bbolean" type="hidden"/>'+
				'<input class="multiple_bbolean_one" type="hidden"/>'+
			
				'<div class="row" contenteditable="false" data-editable="false">' +
				'<div class="col-md-9 offset-md-1">' +	
				'<div class="form-group" contenteditable="false" data-editable="false">'+
				'<div id="check_response_answer" class="check_question_response quiz-explanation" contenteditable="true" data-editable="true" contentDiveditable="true" data-text="Explanation: Enter the explanation for the correct answer (Will be made visible upon Learners answer submission)">Explanation: Enter the explanation for the correct answer (Will be made visible upon Learners answer submission)'+
				'</div>'+
				'</div>'+
				'</div>'+
				'</div>'+
				
				'<div class="form-group bolean_save_answer">'+
				'<ul class="boolean_view_answer">'+
				'<li>'+
				'<div id="toasts"></div>'+
				'<button id="multiple_boolean_success" class="submit_bolean_one_answer bol_transp quiz-submit-answer" type="button"><i class="fa fa-save"></i> Submit Answer</button>'+
				'</li>'+
				'<li>'+
				'<button id="boolean_show_multiple_item" class="bol_transp bollean-multiple-transpot disabled correct-answer-btn" type="button" disabled><i class="fa fa-eye"></i> View Correct Answer</button>'+				   
				'</li>'+
				'</ul>'+
				'</div>'+
			   '</form>'+
	...
	{
		'thumbnail': 'preview/multiple-choice.png',
		'category': '126',
		'html':
		'<div class="row clearfix"  data-editable="false" contenteditable="false">' +
		'<div class="col-md-12">' +
			'<div class="acie_ment quiz-questionnaire tutor_acient tutor_main_checked_select" id='+Math.floor(Math.random() * 100)+'>' + 
			'<form class="select_option_mc multiple_checked_boxing questionnaire-multi-check" contenteditable="false" data-editable="false">'+
			'<input type="hidden" value=0 class="tutor_hide_fill_option" />'+
			'<input type="hidden" value="" name="question_ref_id"  />'+
			   '<h1 contenteditable="false"  data-editable="false">Multiple Answer Choice (select more than one option) Question Template</h1>' +
			   '<div class="form-group"  data-editable="false">'+
			   '<h3 class="quest-heading-attach" rows="1"  data-editable="true" contenteditable="true" contentH3editable="true" data-text="Enter Question Text Here ?">Enter Question Text Here ?</h3>'+
			   '</div>'+
				'<div class="tutor_single_check_option">'+
				'<div class="row">' +
				'<div class="col-md-9 offset-md-1 mobile_screen_size add-more-row">' +	
				'<input type="hidden" name="data_row_number" class="data-row-number" value="1" />'+
				'<div class="form-group">'+
				'<div class="inputGrouping checkGrouping"  data-editable="false" contenteditable="false">'+
				'<input id="option1" name="option" type="checkbox" value="Option One" class="cbox_option" />'+
				'<label class="option1" for="option1"  data-editable="true" contenteditable="true">Option 1</label>'+
				'</div>'+
				'</div>'+
				'</div>'+
				'</div>'+
				...
				'<div class="form-group">'+
				'<div class="inputGrouping checkGrouping"  data-editable="false" contenteditable="false">'+
				'<input id="option2" name="option" type="checkbox" value="Option Two" class="cbox_option" />'+
				'<label class="option2" for="option2"  data-editable="true" contenteditable="true">Option 2</label>'+
				'</div>'+
				'</div>'+
				'</div>'+
				...
				'<div class="col-md-2" contenteditable="false" data-editable="false">' +	
				'<div class="tutor_single_checked_button" contenteditable="false" data-editable="false">'+
				'<a class="btn btn-add-option add-checkbox-row"><i class="fa fa-plus" aria-hidden="true"></i></a>'+
				'</div>'+
				'</div>'+
				...
				'<input class = "multi_box_answer" type="hidden">'+
				'<input class = "multiple_boxes_answers" type="hidden">'+


				'<div class="row">' +
				'<div class="col-md-9 offset-md-1 mobile_screen_size tutor_multiple_check">' +	
				'<div class="form-group"  data-editable="false" contenteditable="false">'+
				'<div  data-editable="true" contenteditable="true" id="check_multiple_response_answer" class="check_question_response quiz-explanation" contentDiveditable="true" data-text="Explanation: Enter the explanation for the correct answer (Will be made visible upon Learners answer submission)">Explanation: Enter the explanation for the correct answer (Will be made visible upon Learners answer submission)'+
				'</div>'+
				'</div>'+
				'</div>'+
				'</div>'+
				...
				'<div class="form-group check_save_answer">'+
				'<ul class="check_view_answer">'+
				'<li>'+
				'<div id="toasts"></div>'+
				'<button id="save_boxing_success" class="submit_check_answer bol_transp quiz-submit-answer" type="button"><i class="fa fa-save"></i> Submit Answer</button>'+
				'</li>'+
				'<li>'+
				'<button id="check_show_item" class="check_view_boxing bol_transp disabled correct-answer-btn" type="button" disabled><i class="fa fa-eye"></i> View Correct Answer</button>'+				   
				'</li>'+
				'</ul>'+
				'</div>'+
				'</form>'+
		...
		{
			'thumbnail': 'preview/single_blank.png',
            'category': '126',
			'html':
				'<div class="row clearfix"  data-editable="false" contenteditable="false">' +		
				  '<div class="col-md-12">' +
				   '<div class="acie_ment quiz-questionnaire subjective_question " id='+Math.floor(Math.random() * 10000)+'>'+
				 
				   '<form name="attempt_blank_answer" class="questionnaire-blank option_boolean_ones" id="add_new_blank_answer">'+
				   '<input type="hidden" value=0 class="tutor_hide_fill_option" />'+
				   '<input type="hidden" value="" name="question_ref_id"  />'+
				   '<div class="form-group">'+
                   '<div class="hidden-group">'+
				   '<h1 id="question_num" contenteditable="false"  data-editable="false">Short/Long Text Answer (Fill in the Blank) Question Template</h1>'+
				   '<input type="hidden" name="question_num"/>'+
				   '</div>'+
				   '</div>'+
				   '<div class="form-group"  data-editable="false" contenteditable="false">'+
                   '<h3  data-editable="true" contenteditable="true" id="question" class="blank-heading-attach question tutor_single_heading_question">Enter Question text Here <span class="iconify tutor_iconify" data-icon="ant-design:line-outlined"></span> ?</h3>'+
				   '</div>'+
				   /* '<div class="form-group">'+
                   '<textarea type="text" name="attemp_answer" id="attemp_answer" class="blank-heading-attach attemp_answer" placeholder="Write A Answer" required></textarea>'+
				   '</div>'+ tutor_form_row*/

                   '<div id="tutor_single_option">'+
				   '<div class="row add-more-row">' +
				   '<input type="hidden" name="data_row_number" class="data-row-number" value="1" />'+
				   '<div class="col-md-9 offset-md-1">' +	
				   '<div class="form-group">'+			
					'<div class="inputGrouping fillGrouping" data-editable="false" contenteditable="false">'+
					'<input id="radio1-blank" name="radio" type="radio" data-id="1" class="radio_mul_checked"/>'+
					'<label class="radio1-blank" for="radio1-blank" data-editable="true" contenteditable="true">Option 1</label>'+
					'</div>'+	
					'</div>'+
					'</div>'+
					'</div>'+
					...
					'<div class="form-group">'+
					'<div class="inputGrouping fillGrouping" data-editable="false" contenteditable="false">'+
					'<input id="radio2-blank" name="radio" type="radio" data-id="2" class="radio_mul_checked"/>'+
					'<label class="radio2-blank" for="radio2-blank" data-editable="true" contenteditable="true">Option 2</label>'+
					'</div>'+
					'</div>'+
					'</div>'+
				   ...
				   '</div>'+
				   '</div>'+
				   '<input class = "sgnl_blank_ip1" type="hidden">'+
				   '<input class = "sgnl_blank_ip2" type="hidden">'+
				   '<div class="form-group">'+
				   '<ul class="sub_view_answer">'+
				   '<li>'+
				   '<div id="toasts"></div>'+
		...
		{
			'thumbnail': 'preview/Multiple-Choice-Test.png',
            'category': '126',
			'html':
				'<div class="row clearfix"  data-editable="false" contenteditable="false">' +		
				  '<div class="col-md-12">' +
				   '<div class="acie_ment quiz-questionnaire multiple_fill_option" id='+Math.floor(Math.random() * 1000)+'>' +
				   '<form id="my_blank_form_multiple" class="my_blank_form_multiple questionnaire-multi-blank">'+
				   '<input type="hidden" value=0 class="hide_fill_option" />'+
				   '<input type="hidden" value="" name="question_ref_id"  />'+
				   '<div class="form-group">'+
                   '<div class="hidden-group">'+
				   '<h1 id="question_num" contenteditable="false"  data-editable="false" class="tutor-multiple-option-heading"> Select From Dropdown (Fill in the Blank - Multiple Blanks) Question Template</h1>'+
				   '<input type="hidden" name="question_num"/>'+
				   '</div>'+
				   '</div>'+
				   '<div class="form-group form-mul-option"  data-editable="false" contenteditable="false">'+
				   '<h3  data-editable="true" contenteditable="true" class="blank-heading-attach tutor_single_heading_question">Enter Question text here <span contenteditable="false" contenteditable="false" ><span class="iconify tutor_iconify" data-icon="ant-design:line-outlined"></span></span> and <span contenteditable="false" contenteditable="false"><span class="iconify tutor_iconify" data-icon="ant-design:line-outlined"></span></span> ? </h3>'+
	
				   '<div id="add_new_blank" class="add_new_blank extra_customer_two new-input-add" contenteditable="false" data-editable="false">'+
				   '<a href="#" class="btn btn-tutor-blank" data-editable="false" contenteditable="false"><i class="fa fa-plus" aria-hidden="true"></i> Add Fill in The Blank</a>'+
				   '</div>'+  		
					 '<div class="customer_blank_dynamic_two"></div>'+		
				   '</div>'+

				   //Start
				    '<div class="row multi-blank-div">'+
					
					'<input type="hidden" value="" data-multiblank-answers="radios" name="multiblank-answers[\'radios\']" />'+
					'<input type="hidden" value="" data-multiblank-answer="radios" name="multiblank-answer[\'radios\']" />'+

					'<div class="col-md-12 offset-md-1 mobile_screen_size tutor_blank_option single_closest tutor_multiple_heading ">' +
					'<div data-editable="false" contenteditable="false">'+
					'<h5 class="crt_option numbering_sort" contenteditable="false" data-editable="false">Blank 1</h5>'+
					'</div>'+	
					'</div>'+	
					...
					'<div class="form-group">'+					
					'<div class="form-group inputGrouping mcq-checked"  data-editable="false" contenteditable="false">'+
					'<input id="radio9" name="radios" type="radio" data-id ="1" class="mcq_checked"/>'+
					'<label class="radio9" for="radio9" data-editable="true" contenteditable="true">Option 1</label>'+
					'</div>'+	
					'</div>'+
					'</div>'+
					'</div>'+
					...
					'<div class="form-group customer_records">'+
					'<div class="form-group inputGrouping mcq-checked" data-editable="false" contenteditable="false">'+
					'<input id="radio10" name="radios" type="radio" data-id ="2" class="mcq_checked"/>'+
					'<label class="radio10" for="radio10" data-editable="true" contenteditable="true">Option 2</label>'+
					'</div>'+	
					'</div>'+
					'</div>'+
				...
				'<input type="hidden" value="" data-multiblank-answers="radios2" name="multiblank-answers[\'radios2\']" />'+
				'<input type="hidden" value="" data-multiblank-answer="radios2" name="multiblank-answer[\'radios2\']" />'+				
				
				'<div class="col-md-12 offset-md-1 mobile_screen_size tutor_blank_option single_closest tutor_multiple_heading">' +
				'<div data-editable="false" contenteditable="false">'+
				'<h5 class="crt_option numbering_sort" contenteditable="false" data-editable="false">Blank 2</h5>'+
				'</div>'+	
				'</div>'+	
				...
				'<div class="form-group">'+					
				'<div class="form-group inputGrouping mcq-checked"  data-editable="false" contenteditable="false">'+
				'<input id="radio11" name="radios2" type="radio" data-id ="1" class="mcq_checked"/>'+
				'<label class="radio11" for="radio11" data-editable="true" contenteditable="true">Option 1</label>'+
				'</div>'+	
				'</div>'+
				'</div>'+
				'</div>'+
				...
				'<div class="form-group customer_records">'+
				'<div class="form-group inputGrouping mcq-checked" data-editable="false" contenteditable="false">'+
				'<input id="radio12" name="radios2" type="radio" data-id ="2" class="mcq_checked"/>'+
				'<label class="radio12" for="radio12" data-editable="true" contenteditable="true">Option 2</label>'+
				'</div>'+	
				'</div>'+
				'</div>'+
				'<div class="col-md-2">' +
				...
				///
				   ...
				   '<div class="customer_records_two">' +
				  	'<div class="row">'+
					'<div class="col-md-10 offset-md-1">'+
					'<input class = "mcqs-answer" type="hidden">'+
					'<input class = "mcqing-answers" type="hidden">'+
					'<div class="form-group" data-editable="false" contenteditable="false">'+
					'<div  data-editable="true" contenteditable="true" id="multiple_response_answer" class="multiple_response_answer question response  quiz-explanation" contentDiveditable="true" data-text="Explanation: Enter the explanation for the correct answer (Will be made visible upon Learners answer submission)">Explanation: Enter the explanation for the correct answer (Will be made visible upon Learners answer submission)'+
					'</div>'+
					'</div>'+
					'</div>'+
					'</div>'+
					...
					'<li>'+
					'<div id="toasts"></div>'+
					'<button id="mul_blank_success" class="submit_mcq_answer quiz-submit-answer" type="button">Submit Answer</button>'+
					'</li>'+
					'<li>'+
					'<button id="multiple_show_item" class="mul-view_answer disabled correct-answer-btn" type="button" disabled>View Correct Answer</button>'+				   
					'</li>'+
					'</ul>'+
				   '</form>'+
			...
			'html':
				'<div class="row clearfix">' +		
				  '<div class="col-md-12">' +
				   ...
				   '<div class="form-group">'+
                   '<div class="hidden-group">'+
				   '<h1 id="question_num">Quiz</h1>'+
				   '<input type="hidden" name="question_num"/>'+
				   '</div>'+
				   '</div>'+
				   '<div class="form-group">'+
                   '<input name="type_question" id="type_question" class="add_type_question form-control" placeholder="Add A Question ?">'+
				   '</div>'+
				   '<div class="form-group">'+
                   '<textarea name="type_answer" id="type_answer" placeholder="Enter Correct Answer"></textarea>'+
				   '</div>'+
				   '</form>'+
				   '</div>' +
					'</div>' +
...
/* 		{
			'thumbnail': 'preview/mcq.png',
            'category': '126',
			'html':
				'<form class="select_option_mc">'+
				'<div class="row clearfix">' +
				'<div class="col-md-12">' +
					'<div class="acie_ment">' + 
				   '<h1>Quiz:</h1>' +
				   '<div class="form-group">'+
                   '<textarea name="type_question_1" id="type_question_1" class="add_question form-control" placeholder="Add A Question ?"></textarea>'+
				   '</div>'+
					'</div>' +
					'</div>' +
					'</div>' +
					...
					'<div class="form-group">'+
					'<ul id="letter-id" class="list-of-mcq tutor-records">'+
					'<li></li>'+
					'</ul>'+
					'<div class="inputGrouping">'+
					'<input id="radio1" name="radio" type="radio" data-id ="1"/>'+
					'<label class="radio1" for="radio1">Yes</label>'+
					'</div>'+	
					'</div>'+
					...
					'</ul>'+
					'<div class="inputGrouping">'+
					'<input id="radio2" name="radio" type="radio" data-id ="2"/>'+
					'<label class="radio2" for="radio2">Yes</label>'+
					'</div>'+	
					'</div>'+
				
					'<div>'+
					'<div onclick="extra_customer()" class="extra_customer"><a href="#" class="btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i></a></div>'+
			...
			'<div class="row clearfix">' +
				'<div class="column full text-center">' +
				   ...
				   '<h1>ACHIEVEMENTS</h1>' +
					...
					'<div class="row">' +
					'<div class="col-md-3">' +
					'<div class="industry_grid" data-noedit>' +
					'<h4>+ 97000</h4>' +
				...
				'</div>' +
					...
					'<div class="industry_grid" data-noedit>' +
					'<h4>+ 25,000</h4>' +
					'<p>ACHIEVEMENTS Asign</p>' +
					'<span>Standard</span>' +
					'</div>' +
					'</div>' +
					'<div class="col-md-3">' +
					'<div class="industry_grid" data-protected>' +
					'<h4>+ 47000</h4>' +
        ...
        }, */
		{
			...
			'html':
					'<div class="row clearfix">' +
						'<div class="column full" data-noedit data-module="my-custom-module" data-module-desc="My Module" data-html="' +
							encodeURIComponent('<h2>My Custom Module</h2> <p> Click setting to change</p>') +
							'" data-settings="">' +
						'</div>' +
					'</div>'
			},
		{
		    ...
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
                        '<div class="display">' +
			    		'<h1>Questions No 1</h1>' +
                		'<p><i>Urdu was declared national language of Pakistan in: </i></p>' +
						'<p>Choose 1 answer</p>'+
						...
						'<div id="block-12" style="padding: 10px;">' +
    						'<label for="option-12" style=" padding: 5px; font-size: 1rem;">'+
		...
		},
		{
		    ...
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
                        '<div class="display">' +
			    		'<h1>Questions No 2</h1>' +
                		'<p><i>The World’s first ATM (Automated teller machine) was built and installed in _______ in 1967: </i></p>' +
						'<p>Choose 1 answer</p>'+
		    ...
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
                        '<div class="display">' +
			    		'<h1>Question No 3</h1>' +
                		'<p><i>Explain what technology we should use in this project and why?</i></p>' +
						'<p>Explain your answer in the box below</p>'+
						'<textarea> </textarea>'+
                        '</div>' +
					'</div>' +
				'</div>'
...
/* 		{
		...
		{
		    'thumbnail': 'preview/Title_and_Body/basic-08.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column half">' +
		...
		{
		    'thumbnail': 'preview/title/basic-09.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/basic-10.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/basic-11.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
...
/* 		{
		...
		{
		    'thumbnail': 'preview/Title_and_Body/basic-14.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column half">' +
...
/* 		{
		...
		/* Video */
		{
		'thumbnail': 'preview/Video_Placeholder/element-video.png',
		'category': '120',
		'html':
        ...
        // Slider Module (Slick)
		{
		'thumbnail': 'preview/Video_Placeholder/element-slider.png',
		'category': '120',
		'html':
			    '<div class="row clearfix">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-02.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
			'thumbnail': 'preview/Title_and_Body/article-04.png',
		    'category': '118',
		    'html':				
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-07.png',
		    'category': '118',
		    'html':				
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-08.png',
		    'category': '118',
		    'html':				
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-09.png',
		    'category': '118',
		    'html':				
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-10.png',
		    'category': '118',
		    'html':				
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-13.png',
		    'category': '118',
		    'html':				
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-16.png',
		    'category': '118',
		    'html':				
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-23.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-25.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-27.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-30.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-31.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column half">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-33.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-35.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-39.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column half">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-40.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-41.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-43.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-46.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-49.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-50.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-51.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-52.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-54.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-55.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-57.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-58.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-59.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-60.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-61.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/article-62.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-02.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-07.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-08.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-09.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
        ...
        {
		    'thumbnail': 'preview/title/header-23.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-25.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full right">' +
		...
		{
		    'thumbnail': 'preview/title/header-26.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-27.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-28.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-32.png',
		    'category': '101',
		    'html':				
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-34.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-38.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-41.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full right">' +
		...
		{
		    'thumbnail': 'preview/title/header-42.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full right">' +
		...
		{
		    'thumbnail': 'preview/title/header-43.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-46.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-47.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-48.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-49.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-50.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-51.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-52.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-53.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-54.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-55.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/title/header-56.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-57.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-58.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-59.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-60.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-61.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-63.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-104.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-105.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column third">' +
		...
		{
		    'thumbnail': 'preview/title/header-106.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/title/header-107.png',
		    'category': '101',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
...
/* 		{
		...
		{
		    'thumbnail': 'preview/Title_and_Body/profile-64.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/profile-69.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Title_and_Body/profile-74.png',
		    'category': '118',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/process/steps-01.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/process/steps-02.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/process/steps-03.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/process/steps-04.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/process/steps-05.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/process/steps-06.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
...
/* 		{
		    'thumbnail': 'preview/process/steps-07.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
...
/* 		{
		    'thumbnail': 'preview/process/steps-08.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column third">' +
		...
		{
		    'thumbnail': 'preview/process/steps-09.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
...
/* 		{
		    'thumbnail': 'preview/process/steps-10.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/process/steps-11.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/process/steps-12.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
 ...
 */
		{
		    'thumbnail': 'preview/process/steps-13.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full center">' +
		...
		{
		    'thumbnail': 'preview/process/steps-14.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/process/steps-15.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column half">' +
		...
		{
		    'thumbnail': 'preview/process/steps-16.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
		...
		{
		    'thumbnail': 'preview/process/steps-26.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/process/steps-27.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/process/steps-28.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column third">' +
		...
		{
		    'thumbnail': 'preview/process/steps-29.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/process/steps-30.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/process/steps-31.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/process/steps-32.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/process/steps-33.png',
		    'category': '106',
		    'html':
				'<div class="row clearfix">' +
					'<div class="column full">' +
 ...
 */
		...
		{
		    'thumbnail': 'preview/Lines_and_Buttons/basic-12.png',
		    'category': '119',
		    'html':
			'<div class="row clearfix">' +
			'<div class="column full">' +
		...
		{
		    'thumbnail': 'preview/Lines_and_Buttons/basic-13.png',
		    'category': '119',
		    'html':
			'<div class="row clearfix">' +
			'<div class="column full">' +
